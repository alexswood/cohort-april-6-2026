...
public class TransactionEnhancer : ITransactionEnhancer
{
        ...
        string currentImportSessionHash)
    {
        ...
        try
        {
            var contextTransactions = await GetSemanticContextTransactionsAsync(
                descriptions, userId, account, DefaultContextLimit, currentImportSessionHash);
...
""";
    ...
    private string CreateUserPrompt(List<string> descriptions)
    {
        var jsonArray = JsonSerializer.Serialize(descriptions);
        return $"Enhance these transaction descriptions:\n\n{jsonArray}";
    }

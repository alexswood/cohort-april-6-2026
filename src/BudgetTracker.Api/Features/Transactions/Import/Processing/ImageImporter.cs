...
public class ImageImporter : IImageImporter
{
    ...
    // Extracts transactions from a bank statement image using GPT-4 Vision
    ...
    // Reads entire image stream into memory as byte array
    ...
    // Sends image to GPT-4 Vision with extraction prompt and returns AI response
    ...
    // Maps file extension to MIME type for vision API
    ...
    // Builds the system prompt for transaction extraction from bank statements
    ...
    // Parses JSON response from AI, validates confidence, and converts to Transaction objects
    ...
    // Extracts and validates individual transaction fields from JSON element
